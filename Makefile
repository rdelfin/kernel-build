...
build-x86-kernel:
	...
	make O=$(OUT_DIR)/x86 ARCH=x86 CROSS_COMPILE=x86_64-linux-gnu- KCONFIG_CONFIG=$(OUT_DIR)/x86/.config -j$(nproc -all) all
...
build-arm64-kernel:
	...
	make O=$(OUT_DIR)/arm64 ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- KCONFIG_CONFIG=$(OUT_DIR)/arm64/.config -j$(nproc --all) all
